...
| `<del>` / `<ins>` | 删除 / 插入的文本 | 版本对比、修订记录 |
| `<kbd>` | 键盘按键 | 「按 `Ctrl+S` 保存」 |
| `<sub>` / `<sup>` | 下标 / 上标 | 化学式、脚注引用、幂运算 |
| `<q>` | 行内短引用 | 自动加引号 |
| `<cite>` | 作品名（书籍/文章/电影） | 引用出处，用斜体呈现 |
| `<dfn>` | 术语定义（首次出现） | 文档中第一次解释某概念 |
| `<samp>` | 程序输出示例 | 终端返回的文本 |
| `<var>` | 变量名 | 数学/代码上下文 |
...
- **一篇页面只写一个 `h1`**（现代 SEO 共识），品牌 logo 里的「标题」用 `span` 或 `div` 别用 `h1`。
- **不要跳级**：`h1 → h3` 会让读屏用户和 SEO 摸不着头脑，宁可把 h3 改成 h2。
- 每个 `<section>` / `<article>` 内部从合适的层级开始，可各自拥有 `h2` 起点。
- 标题层级是「语义顺序」，视觉大小用 CSS 调，别为了「显得小」去用更高一级的标签。
