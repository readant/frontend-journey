...
- 用浏览器控制台、HTML `<script>`、Node.js 三种方式运行 JS
- 分清网页引入 JS 的三种形式，并说出正式项目为何首选外部脚本
- 说出 `<script>` 为什么推荐放 `</body>` 前或加 `defer`
- 知道 Node.js 与浏览器的 API 差异

::: tip 前置要求

需要先掌握 [初识 JavaScript](/03-js/01-foundation/01-javascript-intro)
...
## 符号课堂：`<script>` / `src` / `defer` —— 网页里的"JS 插槽"

网页里写 JS，全靠 `<script>` 这个"插槽"。三个关键符号逐个看：

### 符号一：`<script>` —— "网页里的 JS 插槽"

| 用法     | 示例                                                                                          |
| -------- | --------------------------------------------------------------------------------------------- |
| 基础用法 | `<script>console.log("hi")</script>` —— 标签内直接写 JS                                       |
| 常见错误 | `<script src="app.js">console.log("hi")</script>` —— `src` 与内联混用，内嵌代码被**静默忽略** |
| 正确写法 | `<script src="app.js"></script>` —— 二选一，外部引入时标签内不写代码                          |

### 符号二：`src` —— "把 JS 文件引进来"

| 用法     | 示例                                                       |
| -------- | ---------------------------------------------------------- |
| 基础用法 | `<script src="app.js"></script>` —— 引入同目录的 `app.js`  |
| 常见错误 | `src="app.js"` 但文件路径不对 → 控制台报 404，页面"没效果" |
| 正确写法 | 路径基于**当前 HTML 文件**位置写，确认文件存在再引入       |

### 符号三：`defer` —— "等页面解析完再执行"

| 用法     | 示例                                                             |
| -------- | ---------------------------------------------------------------- |
| 基础用法 | `<script defer src="app.js"></script>` —— 下载不阻塞、解析完执行 |
| 常见错误 | 忘加 `defer` 且脚本放 `<head>` → 操作元素时报 `null`             |
| 正确写法 | 脚本放 `</body>` 前，或加 `defer` 放 `<head>` 里                 |
...
1. **想最快跑一句 JS（不用建文件），用哪种方式？**
   - A. 浏览器控制台（F12 → Console）
   - B. 新建 HTML 文件
   - C. Node.js 文件
2. **正式项目引入 JS，推荐哪种形式？**
   - A. 内联脚本 `<script>...</script>`
   - B. 外部脚本 `<script src="app.js"></script>`
   - C. 事件属性 `onclick="..."`
3. **`<script src="app.js">console.log("hi")</script>` 会怎样？**
   - A. 先执行内联，再加载外部
   - B. 内联代码被忽略，只加载外部
   - C. 报语法错误
4. **脚本要操作页面元素、防 `null`，正确姿势是？**
   - A. 放 `<head>` 里
   - B. 放 `</body>` 前，或加 `defer`
   - C. 放到任意位置
...
<details>
  ...
  <ol>
    <li>A —— 控制台零文件、最快</li>
    <li>B —— 外部脚本便于维护与复用，浏览器还会缓存</li>
    <li>B —— 用了 src 就不要再写内联代码，内嵌部分被静默忽略</li>
    <li>B —— 保证脚本执行时元素已解析完毕</li>
  </ol>
